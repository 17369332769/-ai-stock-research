...
audit-training-data: ## 审计日线、分钟线、复权口径与历史成分
	$(COMPOSE) run --rm trainer python -m services.prediction.training.cli --audit-only

.PHONY: train-models
train-models: ## 泄漏测试通过后训练两个日频模型并登记 candidate
	$(COMPOSE) run --rm trainer python -m services.prediction.training.cli --activate-passing
