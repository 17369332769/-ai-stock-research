...
"公告 vs 新闻"由 provider 区分，两种口径不混（spec §5.2：不做静默备用源）。
────────────────────────────────────────────────────────────────────────────
...
# 一次 quote 请求最多带多少 symbol（避免 URL 过长；自选股规模远小于此）
...
class OpenBBHttpGateway:
    ...
    ``OpenBBHttpGateway`` 是无状态的（除了 httpx 连接池）：不缓存、不落库、不做备用源。
    """
    ...
    async def _results(self, route: str, params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if status >= 500:
            raise ProviderUnavailable(f"OpenBB 服务端错误（HTTP {status}）：{route}")
        if status >= 400:
